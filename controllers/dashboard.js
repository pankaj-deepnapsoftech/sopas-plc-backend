...
// PLC machine DM words / aggregated payload POST
exports.createPlcMachineData = async (req, res) => {
  try {
    const {
      device_id,
      timestamp,
    ...
    if (!ts) {
      return res
        .status(400)
        .json({ success: false, message: "timestamp or time is required." });
    }
    ...
    const data = await require("../models/machineStatus").PlcMachine.create(
      payload
    );
    return res
      .status(201)
      .json({ success: true, message: "PLC machine data saved", data });
  } catch (error) {
    res
      .status(500)
      .json({
...
// PLC machine DM words GET: ?device_id=... (optional)
exports.getPlcMachineData = async (req, res) => {
  try {
    const { device_id } = req.query;
    const filter = device_id ? { device_id } : {};
    const data = await require('../models/machineStatus').PlcMachine.find(filter).sort({ timestamp: -1 });
